...
describe('IdFactory', () => {
  it('should revert because authority is Zero address', async () => {
    ...
    await expect(IdFactory.connect(deployerWallet).deploy(ethers.constants.AddressZero)).to.be.revertedWithCustomError(IdFactory, 'ZeroAddress');
  });
  ...
  it('should revert because wallet of identity cannot be Zero address', async () => {
    ...
    await expect(identityFactory.connect(deployerWallet).createIdentity(ethers.constants.AddressZero, 'salt1')).to.be.revertedWithCustomError(identityFactory, 'ZeroAddress');
  });
  ...
  describe('link/unlink wallet', () => {
    describe('linkWallet', () => {
      it('should revert for new wallet being zero address', async () => {
        ...
        await expect(identityFactory.connect(aliceWallet).linkWallet(ethers.constants.AddressZero)).to.be.revertedWithCustomError(identityFactory, 'ZeroAddress');
      });
      ...
      it('should revert for new wallet being already to a token identity', async () => {
        ...
        await expect(identityFactory.connect(bobWallet).linkWallet(tokenAddress)).to.be.revertedWithCustomError(identityFactory, 'TokenAddress');
      });
    ...
    describe('unlinkWallet', () => {
      it('should revert for wallet to unlink being zero address', async () => {
        ...
        await expect(identityFactory.connect(aliceWallet).unlinkWallet(ethers.constants.AddressZero)).to.be.revertedWithCustomError(identityFactory, 'ZeroAddress');
      });
