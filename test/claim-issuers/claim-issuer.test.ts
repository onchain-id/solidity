  ...
  describe('revokeClaim (deprecated)', () => {
    describe('when calling as a non MANAGEMENT key', () => {
      it('should revert for missing permissions', async () => {
        const { claimIssuer, aliceWallet, aliceClaim666, identityImplementation } = await loadFixture(deployIdentityFixture);

        await expect(claimIssuer.connect(aliceWallet).revokeClaim(aliceClaim666.id, aliceClaim666.identity)).to.be.revertedWithCustomError(identityImplementation, 'SenderDoesNotHaveManagementKey');
      });
    });
  ...
  describe('revokeClaimBySignature', () => {
    describe('when calling as a non MANAGEMENT key', () => {
      it('should revert for missing permissions', async () => {
        ...
        await expect(claimIssuer.connect(aliceWallet).revokeClaimBySignature(aliceClaim666.signature)).to.be.revertedWithCustomError(identityImplementation, 'SenderDoesNotHaveManagementKey');
      });
    });
