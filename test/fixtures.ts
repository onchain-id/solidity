...
export async function deployIdentityFixture() {
  ...
  await claimIssuer.connect(claimIssuerWallet).addKey(
    ethers.keccak256(
      AbiCoder.defaultAbiCoder().encode(['address'], [claimIssuerWallet.address]),
    ),
    3,
    1,
  );
  ...
  aliceClaim666.id = ethers.keccak256(AbiCoder.defaultAbiCoder().encode(['address', 'uint256'], [aliceClaim666.issuer, aliceClaim666.topic]));
