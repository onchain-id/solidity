...
function shouldBehaveLikeERC734({ errorPrefix, identityIssuer, identityOwner, anotherAccount }) {
  ...
  describe('addKey', function () {
    context('when sender has no management key', function () {
      it('reverts for insufficient privileges', async function () {
        await expectRevert(
          this.identity.addKey(bufferToHex(keccak256(abi.rawEncode(['address'], [identityOwner]))), 1, 1, { from: identityOwner }),
    ...
    context('when sender has a management key', function () {
      context('when key is already registered with this purpose', function () {
        it('reverts for conflict', async function () {
          await expectRevert(
            this.identity.addKey(bufferToHex(keccak256(abi.rawEncode(['address'], [identityIssuer]))), 1, 1, { from: identityIssuer }),
      ...
      context('when key is already registered but not with this purpose', function () {
        it('adds the purpose to the key and emits a KeyAdded event', async function () {
          const { logs } = await this.identity.addKey(bufferToHex(keccak256(abi.rawEncode(['address'], [identityIssuer]))), 3, 1, { from: identityIssuer });

          expectEvent.inLogs(logs, 'KeyAdded', {
            key: bufferToHex(keccak256(abi.rawEncode(['address'], [identityIssuer]))),
            purpose: '3',
            keyType: '1',
          });

          const key = await this.identity.getKey(bufferToHex(keccak256(abi.rawEncode(['address'], [identityIssuer]))));

          expect(key.key).to.equal(bufferToHex(keccak256(abi.rawEncode(['address'], [identityIssuer]))));
      ...
      context('when key is not registered', function () {
        it('adds the key with the purpose and emits a KeyAdded event', async function () {
          const { logs } = await this.identity.addKey(bufferToHex(keccak256(abi.rawEncode(['address'], [identityOwner]))), 1, 1, { from: identityIssuer });
          ...
          const key = await this.identity.getKey(bufferToHex(keccak256(abi.rawEncode(['address'], [identityOwner]))));

          expect(key.key).to.equal(bufferToHex(keccak256(abi.rawEncode(['address'], [identityOwner]))));
          expect(key.keyType).to.be.bignumber.equal('1');
  ...
  describe('getKeysByPurpose', function () {
    ...
    context('when identity has no key of this purpose, but used to have one', function () {
      beforeEach('remove key', async function () {
        await this.identity.removeKey(bufferToHex(keccak256(abi.rawEncode(['address'], [identityIssuer]))), 1, { from: identityIssuer });
      });
    ...
    context('when identity has key of this purpose', function () {
      beforeEach('add keys', async function () {
        await this.identity.addKey(bufferToHex(keccak256(abi.rawEncode(['address'], [identityOwner]))), 1, 1, { from: identityIssuer });

        await this.identity.addKey(bufferToHex(keccak256(abi.rawEncode(['address'], [anotherAccount]))), 3, 1, { from: identityIssuer });
      });
  ...
  describe('keyHasPurpose', function () {
    context('when identity has no such key', function () {
      it('returns false', async function () {
        await expect(this.identity.keyHasPurpose(bufferToHex(keccak256(abi.rawEncode(['address'], [anotherAccount]))), 1)).to.eventually.be.false;
      });
    });
    ...
    context('when identity has the key with the purpose', function () {
      ...
      it('returns true', async function () {
        await expect(this.identity.keyHasPurpose(bufferToHex(keccak256(abi.rawEncode(['address'], [anotherAccount]))), 3)).to.eventually.be.true;
      });
    });

    context('when identity has the key with a management purpose', function () {
      it('returns true', async function () {
        await expect(this.identity.keyHasPurpose(bufferToHex(keccak256(abi.rawEncode(['address'], [identityIssuer]))), 3)).to.eventually.be.true;
      });
    });
  });
  ...
  describe('removeKey', function () {
    context('when sender has no management key', function () {
      it('reverts for insufficient privileges', async function () {
        await expectRevert(
          this.identity.removeKey(bufferToHex(keccak256(abi.rawEncode(['address'], [identityIssuer]))), 1, { from: identityOwner }),
          'Permissions: Sender does not have management key',
        );
      });
    });
    ...
    context('when sender has a management key', function () {
      context('when the key is not registered', function () {
        it('reverts for non-existing key', async function () {
          await expectRevert(
            this.identity.removeKey(bufferToHex(keccak256(abi.rawEncode(['address'], [identityOwner]))), 1, { from: identityIssuer }),
      ...
      context('when the key is registered but not with the removed purpose', function () {
        it('reverts for non-existing key purpose', async function () {
          await expectRevert(
            this.identity.removeKey(bufferToHex(keccak256(abi.rawEncode(['address'], [identityIssuer]))), 3, { from: identityIssuer }),
      ...
      context('when the key is registered with more purpose than the removed one', function () {
        beforeEach('add key', async function () {
          await this.identity.addKey(bufferToHex(keccak256(abi.rawEncode(['address'], [identityOwner]))), 3, 1, { from: identityIssuer });

          await this.identity.addKey(bufferToHex(keccak256(abi.rawEncode(['address'], [identityOwner]))), 4, 1, { from: identityIssuer });
        });
        ...
        it('removes the purpose from the key and emits a KeyRemoved event', async function () {
          ...
          const { logs } = await this.identity.removeKey(bufferToHex(keccak256(abi.rawEncode(['address'], [identityOwner]))), 3, { from: identityIssuer });
          ...
          await expect(this.identity.getKeyPurposes(bufferToHex(keccak256(abi.rawEncode(['address'], [identityOwner])))))
