...
contract('Identity', function ([identityIssuer, identityOwner, claimIssuer, anotherAccount]) {
  describe('Identity', function () {
    beforeEach(async function () {
      ...
      this.identityFactory = await IdentityFactory.new(this.identity.address, { from: identityIssuer });
      await this.identityFactory.createIdentity(identityIssuer);
      this.identity = await Identity.at(await this.identityFactory.IdentityAddresses(0));
    });
    ...
    it('Should be a Cloned Identity', async function () {
      expect(await this.identityFactory.isClonedIdentity(this.identity.address)).to.equals(true);
    });
    ...
    it('Should returns Identities', async function () {
      ...
      expect(await this.identityFactory.getIdentities()).to.include(identity0, identity1, identity2);
    });
    ...
    it('Should set a new LibraryAddress', async function () {
      ...
      expect(await this.identityFactory.libraryAddress()).to.equals(newIdentity.address);
    });

    it('Should not be able to set an Identity', async function () {
      const loadedIdentityWithAnotherAccount = await Identity.at(await this.identityFactory.IdentityAddresses(0));
      await expect(loadedIdentityWithAnotherAccount.set(anotherAccount, { from: anotherAccount })).to.be.rejectedWith(Error);
    });
  });
});
