...
describe('Identity', () => {
  it('should revert when attempting to initialize an already deployed identity', async () => {
    ...
    await expect(aliceIdentity.connect(aliceWallet).initialize(aliceWallet)).to.be.revertedWith('Initial key was already setup.');
  });
  ...
  it('should revert because interaction with library is forbidden', async () => {
    ...
    await expect(identityImplementation.connect(aliceWallet).initialize(deployerWallet))
